document.addEventListener("DOMContentLoaded", () => {
  ...
  function drawDisk() {
    ...
    ctx.beginPath(); ctx.arc(centerX, centerY, outerRadius, 0, 2 * Math.PI); ctx.fill();
    ctx.fillStyle = "#495057";
    ctx.beginPath(); ctx.arc(centerX, centerY, innerRadius, 0, 2 * Math.PI); ctx.fill();
    ctx.fillStyle = "#212529";
    ctx.beginPath(); ctx.arc(centerX, centerY, centerHoleRadius, 0, 2 * Math.PI); ctx.fill();
